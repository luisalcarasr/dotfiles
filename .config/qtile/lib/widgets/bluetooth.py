...
class Bluetooth(base._TextBox):
    ...
    def _adapter_signal_received(self, interface_name, changed_properties, _invalidated_properties):
    ...
    @property
    def connected(self):
        return True not in [not not device.connected for device in self.devices]

    def mouse_enter(self, *args, **kwargs):
        name, *_ = [device.name for device in self.devices if device.connected]
        length = len([device for device in self.devices if device.connected])
        if length is 1:
            if name is not None:
